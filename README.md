...
10. Cached conda and pip packages are not copied to the new folder. MKL package is uninstalled. The `tar.bz2` package with latest MKL version is determined in conda cache folder and copied to the new folder. Batch script that can install MKL package on the end-user computer is added.
11. About patches. Some patches are applied to the files specified. If the file has specified hash then the patch is applied. If it doesn't then warning nessage is shown.
